...
// Supabase removido — dados via JSON externo (DATA_SOURCES em src/services/dataService.ts)
...
// Auth via API_AUTH_TOKEN do env (sem Supabase)
// TODO: substituir por solução de autenticação definitiva da empresa
function requireAuth(req: Request, res: Response, next: NextFunction): void {
  const apiAuthToken = process.env.API_AUTH_TOKEN;
  if (!apiAuthToken) { next(); return; } // modo dev: sem token configurado = acesso livre
...
app.get('/api/dashboard/finance/receivable', async (req, res) => {
  ...
  if (!q) return;
  try {
    const data = await financeRepo.getAccountsReceivableOpen({ bucket: q.bucket, customerId: q.customerId });
    res.json(data);
  } catch (e) {
...
app.get('/api/dashboard/finance/payable', async (req, res) => {
  ...
  if (!q) return;
  try {
    const data = await financeRepo.getAccountsPayableOpen({ bucket: q.bucket, category: q.category });
    res.json(data);
  } catch (e) {
...
app.get('/api/products', async (req, res) => {
  ...
  try { res.json(await productsRepo.listProducts(...q)); }
...
// TODO: inventory_movements não tem JSON externo definido ainda — endpoint retorna array vazio
app.get('/api/stock/movements', async (req, res) => {
  const q = parseQuery(qLimit50, req.query, res); if (!q) return;
  try {
    const { storeGetAll } = await import('../services/dataService');
    const all = await storeGetAll('stockReservations'); // proxy até URL dedicada ser fornecida
...
// ── BI — KPIs diários ─────────────────────────────────────────────────────────
// TODO: endpoints /api/bi/* precisam de URLs JSON dedicadas — atualmente buscam do salesByDay/salesByCustomer como proxy
...
app.get('/api/bi/ar-aging', async (_req, res) => {
  try {
    ...
    const sorted = (data as Record<string, unknown>[]).sort((a, b) => Number(b['days_overdue'] ?? 0) - Number(a['days_overdue'] ?? 0));
    res.json(sorted);
